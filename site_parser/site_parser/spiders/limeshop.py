...
class LimeshopSpider(scrapy.Spider):
    name = "limeshop"
    allowed_domains = ["lime-shop.com"]
    start_urls = ['https://lime-shop.com/api/section/sumki?page=1&size=1000',
    ...
    def parse(self, response):
        ...
        for product in table:
            ...
            data['item_name'] = product['cells'][0]['entity']['name']
            data['item_price'] = int(product['cells'][0]['entity']['models'][0]['skus'][0]['price'])
            data['item_image'] = product['cells'][0]['entity']['models'][0]['photo']['url']
            entity = product['cells'][0]['entity']['code']
            model = product['cells'][0]['entity']['models'][0]['code']
            data['source_url'] = f'https://lime-shop.com/ru_ru/product/{entity}-{model}'
            yield data
